...
ggplot(data = wi, mapping = aes(y = log(ADT_029), x =YEAR_BUILT_027)) +geom_point() + geom_smooth()
...
wi %>% group_by(YEAR_BUILT_027) %>% summarise(prop = mean(rate=="good")) %>% ggplot(mapping = aes(YEAR_BUILT_027, y = prop)) + geom_point()
...
ggplot(data = Q, mapping = aes(y = DECK_PROTECTION_108C, x = DECK_COND_058)) +geom_point() + geom_smooth(method = "loess", span = .7) + geom_jitter()
...
#Amount of Traffic and Building materials
#Add colors associated with time intervals
Q$colors <- rep("", nrow(Q))
Q$YEAR_BUILT_027 <- as.numeric(Q$YEAR_BUILT_027)
i=1
for(i in 1:nrow(Q)+1){
  if(Q[i,9] < 1908){ #before Ford releases Model T
    Q$colors[i] = "#000000"
  }
  if(Q[i,9] < 1919 && Q[i,9] >= 1908){ #World War I ends
    Q$colors[i] = "#E69F00"
  }
  if(Q[i,9] < 1929 && Q[i,9] >= 1919){ #before Great Depression
    Q$colors[i] = "#56B4E9"
  }
  if(Q[i,9] < 1939 && Q[i,9] >= 1929){ #World War II begins
    Q$colors[i] = "#009E73"
  }
  if(Q[i,9] < 1945 && Q[i,9] >= 1939){ #World War II ends
    Q$colors[i] = "#F0E442"
  }
  if(Q[i,9] < 1973 && Q[i,9] >= 1945){ #Vietnam War ends
    Q$colors[i] = "#0072B2"
  }
  if(Q[i,9] < 2000 && Q[i,9] >= 1973){ #End of 20th century
    Q$colors[i] = "#D55E00"
  }
  if(Q[i,9] >= 2000){ #21st century
    Q$colors[i] = "#CC79A7"
  }
}  
#colors picked for people who are color-blind

#This plot shows daily trafic vs. bridge condition colored by years
ggplot(data = Q, mapping = aes(y = ADT_029, x = SUPERSTRUCTURE_COND_059, col = colors)) +geom_point() + geom_smooth(method = "loess", span = .7) + geom_jitter()
  #Still want to use the color codes as the actual colors in the legend on the plot



#Code for bringing in new state
illin = filter(x, STATE_CODE_001 == 17) #Illinois
hist(illin$LAT_016 %>% min2dec %>% as.numeric)
min2dec(illin$LAT_016[1])
hist(illin$LAT_016 %>% min2dec %>% as.numeric)
illin = mutate(illin,lat = min2dec(LAT_016), lon = min2dec(LONG_017))
ggplot(data = illin) +geom_point(mapping = aes(y = lat, x = lon))

#fix up the latitude and longitude
illin = filter(illin,lon<120)
illin = filter(illin,lon>80)
illin = filter(illin,lat>35)
illin = filter(illin,lat<80)
ggplot(data = illin) +geom_point(mapping = aes(y = lat, x = lon))

#Quick maps to get a feel for the state
ggplot(data = illin) +geom_point(mapping = aes(y = lat, x = lon,col =TOLL_020))
ggplot(data = illin) +geom_point(mapping = aes(y = lat, x = lon,col =YEAR_BUILT_027))


# cond "condition" is the minimum of the given ratings. 
illin = mutate(illin, cond = pmin(SUPERSTRUCTURE_COND_059, SUBSTRUCTURE_COND_060, CHANNEL_COND_061,CULVERT_COND_062, 
                                  na.rm = T))
illin$rate = rateIt(illin$cond)
table(illin$cond)
table(illin$rate)
illin = filter(illin, cond>1)
ggplot(data = illin, mapping = aes(y = log(ADT_029), x =YEAR_BUILT_027, col = rate)) +geom_point() + geom_smooth()
illin %>% group_by(YEAR_BUILT_027) %>% summarise(prop = mean(rate=="good")) %>% ggplot(mapping = aes(YEAR_BUILT_027, y = prop)) + geom_point()
map = ggplot(data = illin, mapping = aes(y = lat, x = lon))
map + geom_point(aes(col=rate))+ scale_colour_brewer(palette = "Spectral")  
# where are these bad roads?!!??
ggplot(data = illin, mapping = aes(x = rate, y = log(ADT_029))) + geom_boxplot()
colnames(illin)
# use a data playground!
illin = mutate(illin, fips = STATE_CODE_001*1000+COUNTY_CODE_003)
illin = illin %>% mutate(fips = STATE_CODE_001*1000+COUNTY_CODE_003)
illin = illin %>% mutate(good = (rate == "good"))
table(illin$good)
dat = illin %>% group_by(fips) %>% summarize(propGoodRoads = mean(good))
...
#look at wearing surface (item 108A)
#check the proportion of the bridges that do not have a deck, denoted "N"
#illin = filter(x, STATE_CODE_001 == 55) check to run around line 131
Q = illin
Q = filter(Q, SURFACE_TYPE_108A != "N") #remove "not applicable"
Q = filter(Q, SURFACE_TYPE_108A != "9") #remove "other
ggplot(data = Q) +geom_point(mapping = aes(y = lat, x = lon, col =SURFACE_TYPE_108A)) + scale_x_reverse()
...
#compare "toll road type" to "deck condition"
ggplot(data = Q, mapping = aes(y = TOLL_020, x = DECK_COND_058)) +geom_point() + geom_smooth(method = "loess", span = .7) + geom_jitter()

#compare "avg daily trafic" to "deck condition"
ggplot(data = Q, mapping = aes(y = ADT_029, x = DECK_COND_058, col = SUPERSTRUCTURE_COND_059)) +geom_point() + geom_smooth(method = "loess", span = .7)

#compare "avg daily trafic" to "surface type"
ggplot(data = illin, mapping = aes(y = ADT_029, x = SURFACE_TYPE_108A, col = DECK_COND_058)) +geom_point() + geom_smooth(method = "loess", span = .7)
...
#Amount of Traffic and Building materials
#Add colors associated illinth time intervals
Q$colors <- rep("", nrow(Q))
Q$YEAR_BUILT_027 <- as.numeric(Q$YEAR_BUILT_027)
i=1
for(i in 1:nrow(Q)+1){
